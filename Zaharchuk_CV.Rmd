...
My research interests lie at the intersection of cognitive psychology and sociolinguistics. I am particularly interested in the production and comprehension of dialectal variation, bilingual language use, and the neural and cognitive representations of contact languages.
...
education %>% 
  detailed_entries(degree, when, university, where, c(advisor, thesis, award_1, award_2), .protect = FALSE)
```
...
experience <- experience %>%
  arrange(desc(begin), end) %>%
  mutate(where = as.character(glue("{city}, {region}")),
         when = as.character(glue("{month(begin, label = TRUE, abbr = TRUE)} {year(begin)} - 
                                  {if_else(!is.na(end), paste(month(end, label = TRUE, abbr = TRUE), 
                                  year(end)), 'present')}")))
...
Programming: R, MATLAB, \LaTeX, SQL, Python, E-Prime, Praat
